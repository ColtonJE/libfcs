...
static void fcs_prepare_usage(void)
{
	...
	printf("%-32s  %s", "-V|--version", "Show version information\n");
	printf("--------------------------------------------\n");
	printf("\n");
}

/**
 * fcs_get_version() - Get the version information
 *
 * Return: Pointer to a string containing the version information
 */
static const char *fcs_get_version(void)
{
	static char version_str[128];
	snprintf(version_str, sizeof(version_str),
		 "Version: %s, Git SHA: %s", FCS_PROJECT_VERSION,
		 FCS_GIT_HASH);
	return version_str;
}
			   ...
			   char *key_data_file, char *key_obj_file, bool verbose)
{
	...
	if (key_id <= 0 || key_usage == 0 || key_data_file == NULL || key_obj_file == NULL) {
